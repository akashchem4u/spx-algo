#!/usr/bin/env python3
"""
Probe: Strong-bear abstain gate.

Finding: SSR 0–24 (strong bear) calls have only 30.8% accuracy in the 2yr window
vs 52–53% for moderate bear (25–44). These extreme-low-score days coincide with
maximum market pessimism — the model's bear signals are ALL firing simultaneously,
often right before a mean-reversion bounce.

Mechanism: When SSR ≤ 24, the market has already priced in extreme fear.
Fade-the-fear dynamics dominate on a non-gap-down extreme-bear day.
The gap-down abstain gate already removes gap-down bear calls (largest subset);
remaining SSR 0-24 calls are non-gap-down extreme pessimism days.

Variants:
  A — Abstain bear calls when SSR ≤ 24 (strong bear zone)
  B — Abstain bear calls when SSR ≤ 20 (tighter gate)
  C — Abstain bear calls when SSR ≤ 30 (broader gate, includes some mod-bear)
"""
...
def run_probe():
    print("[strong_bear] Fetching 2y data …")
    ...
    for i in range(eval_start,n-1):
        ...
        is_strong_bear = bear and sc <= 24
        if is_strong_bear: n_strong_bear += 1

        rows_base.append({"correct":correct,"score":sc})

        # Variant A: abstain bear calls when SSR ≤ 24
        if bear and sc <= 24:
            pass  # abstain
        else:
            rows_a.append({"correct":correct})

        # Variant B: abstain bear calls when SSR ≤ 20
        if bear and sc <= 20:
            pass  # abstain
        else:
            rows_b.append({"correct":correct})

        # Variant C: abstain bear calls when SSR ≤ 30
        if bear and sc <= 30:
            pass  # abstain
        else:
    ...
    # VIX distribution of strong bear calls
    print(f"\n{'─'*70}")
    print(f"  Strong bear (SSR ≤24) directional calls: {n_strong_bear}/{bt} ({round(n_strong_bear/bt*100,1)}% of calls)")
    print(f"{'─'*70}")
    print(f"  Baseline:                          {bh}/{bt} = {ba}%")
    print(f"  +Abstain bear ≤24 (Variant A):     {ah}/{at} = {aa}%  Δ={aa-ba:+.1f}pp  coverage:{round(at/bt*100,1)}%")
    print(f"  +Abstain bear ≤20 (Variant B):     {bxh}/{bxt} = {bxa}%  Δ={bxa-ba:+.1f}pp  coverage:{round(bxt/bt*100,1)}%")
    print(f"  +Abstain bear ≤30 (Variant C):     {ch}/{ct} = {ca}%  Δ={ca-ba:+.1f}pp  coverage:{round(ct/bt*100,1)}%")
    print(f"{'─'*70}")
